...
export function ExpenseTransactionForm({
  ...
  const toggleCommonTag = (tag: Tag) => {
    ...
    // Adding a new abs-mode add-on will switch the form out of simple mode.
    // Copy the main amount into the sub-entry so the base amount is preserved.
  ...
  const toggleCommonAmtType = (tagId: number) => {
    ...
    if (currentEntry.amtType === 'pct') {
      ...
      // abs → pct: compute pct from current abs value
      setActiveCommons(prev => prev.map(c => {
        if (c.tagId !== tagId) return c
