...
describe('migrations', () => {
  ...
  describe('runMigrations', () => {
    ...
    it('parses db_version correctly', async () => {
      ...
      mockQueryOne.mockResolvedValue({ value: '17' })
    ...
    it('runs many statements for full migration', async () => {
      ...
      // Full migration (v1-v17): each migration joins statements into one execSQL call
      // v1 = 1 call, v2-v17 = 2 calls each (migration + version update) = 1 + 16*2 = 33
      expect(mockExecSQL.mock.calls.length).toEqual(33)
    })
